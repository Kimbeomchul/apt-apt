...
**전수 분석과 검증된 입지 급지 평가는 아직 아닙니다.** 초기 자료를 실제 현재 매물이나 공인 평가로 표현하지 않습니다. 가격군은 선택 평형의 금액 구간이며 입지 등급이 아닙니다. 정책 엔진은 일반·생애최초 무주택자 은행 주담대 참고 시나리오이며 현행 규정 전체 검증·은행 승인 판단을 대체하지 않습니다. 현재 주택 보유와 과거 보유 이력을 별도로 선택하고, 생애최초 변경에 따른 예산·LTV·부족 현금·월 상환액을 비교합니다.
...
별도 연구 원장에는 공식 단지 4,860개와 국토부 웹 CSV 매매 원본 447,834행을 확보했습니다. API를 호출하지 않고 파일 다운로드로 수집했습니다. 만 연식, 59/84㎡ 3·6·12개월 가격 비교, 정확한 면적별 월 집계, 확인된 커뮤니티 시설 근거를 저장합니다. 앱에 표시되는 초기 7개 단지와는 별도입니다.

시작점: [실제 수집 현황](docs/APARTMENT_BIGDATA.md) · [공식 단지 MD](docs/APARTMENT_RESEARCH_OFFICIAL.md) · [재현 명령과 Astra 인수인계](docs/APARTMENT_RESEARCH_HANDOFF.md) · [미완료 우선순위](docs/ROADMAP.md). 원본과 SQLite는 C드라이브 공간 부족으로 `D:/apt-research-work/`에 보관했습니다.
...
`src/finance.js`는 `data/policy.json`의 LTV·DSR·가격별 상한을 직접 읽습니다. 정책 변경 시 JSON과 경계 테스트를 함께 검토하세요. 날짜별 경과규정과 디딤돌·보금자리론 자격 엔진은 아직 구현하지 않았습니다.

- 확인 자료: 2025-10-15 대책, 2026-04-01 관리방안, 2026-06-30 규제지역 추가 지정.
- DTI, 담보평가·방공제, 생애최초 인정 예외·정책모기지, 보유주택 처분, 전입·취득 제한 및 경과규정은 별도 검토.
