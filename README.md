...
- 공식 마스터 전체 4,860개 단지(서울 1,339 / 경기 3,521), 세대수·준공연도·평형별 참고가격·기준일·출처
- 전체 목록 검색과 24개 단위 페이지 이동, 모바일·PC 글자 크기 확대
- 6억 미만 / 6~8억 / 8~11억 / 11~15억 가격대별 탐색 및 구간별 단지 수
- 펼친 입력 영역 레이아웃 수정, 14~16px 본문 중심의 네이비·청록 테마와 모바일 탐색 개선
- 나 혼자 / 부부 합산 탭, 각각의 소득·현금·부채 입력 및 합산 자금 요약
- 본인·배우자 현금 증여세 참고 계산, 과거 공제 이력·혼인/출산 공제 가정, 세후 자금 반영
- 신용대출 전액 상환·해지 상태 구분, 계산 보류 시 예산 필터의 목록 소실 방지
- 시·군·구 및 가격 자료 품질 필터, 금융정보를 제외한 검색 조건 공유
- 단지 상세의 3·6·12개월 거래 중위값·범위·거래 수·직전 기간 대비 비교 (상세 진입 시 로드)
- 모바일 하단 예산 요약과 입력/결과 바로가기, 빈 증여 입력 시 오류 대신 계산 보류
...
부부·증여·신용대출의 적용 범위와 공식 근거: [구현 안내](docs/HOUSEHOLD_GIFT_CREDIT.md).
...
공식 단지 4,860개와 국토부 웹 CSV 매매 원본 447,834행을 확보했습니다. API를 호출하지 않고 파일 다운로드로 수집했습니다. 공식 마스터 전체를 앱에 연결했으며, 최근 12개월 59/84㎡ 참고가격이 있는 단지는 3,274개입니다. 가격이 없는 단지도 검색·상세·비교가 가능합니다. 전체 현존/신축/임대 단지의 누락 검증까지 완료한 것은 아닙니다.

지역별 검증 JSON을 갱신한 뒤 `node scripts/export-app.mjs`로 앱 데이터를 재생성하고 `npm.cmd run check`로 검증합니다. 최근 3개월 5건 이상이면 해당 중위값, 그 미만이면 6개월, 6개월 거래가 없으면 12개월 중위값을 사용합니다. 5건 미만은 소표본이며 거래가 없으면 null을 유지합니다. 집계 기준일은 2026-09-16입니다.
...
현재 앱 JSON은 공식 연구 원장의 주소·명칭 일치 거래 집계에서 생성합니다. 해제·직거래·중복 의심·식별 충돌 건은 제외하며, 신고 지연과 이후 정정은 추가 갱신이 필요합니다.
