...
try{
  ...
  await run("document.querySelector('[data-action=save-scenario]').click()");assert.equal(await run("document.querySelectorAll('[data-load-scenario]').length"),1,'scenario saved');
  await run("document.querySelector('[data-household=couple]').click();document.querySelector('[data-action=save-scenario]').click();document.querySelector('[data-action=compare-scenarios]').click()");assert.equal(await run("document.querySelectorAll('.scenario-table tbody tr').length"),2,'saved scenarios compare');await run("document.querySelector('#close-modal').click()");
  ...
  await run("document.querySelector('[data-detail]').click()");assert.equal(await run("document.querySelectorAll('.data-confidence').length"),1,'trust panel rendered');assert.equal(await run("document.querySelectorAll('.affordability-explain').length"),1,'affordability explanation rendered');assert.ok(await run("document.querySelector('.budget-line').getAttribute('aria-label').includes('현재 집값')"));assert.ok(await run("document.querySelector('.planner-output').textContent.includes('계약금')"));
