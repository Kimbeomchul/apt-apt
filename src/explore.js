...
export function readSearchParams(search){
  ...
  return {region:allowed('region',['all','서울','경기'],'all'),district:(p.get('district')||'all').slice(0,80),area:allowed('area',['59','84'],'84'),priceBand:allowed('band',priceBands.map(b=>b.id),'all'),sort:allowed('sort',['price','price-desc','households','newest'],'price'),quality:allowed('quality',['all','known','sufficient'],'all'),search:(p.get('q')||'').slice(0,100)};
}
...
export function sharedSearchUrl(base,state){
  ...
  for(const [key,value,defaultValue] of [['region',state.region,'all'],['district',state.district,'all'],['area',state.area,'84'],['band',state.priceBand,'all'],['sort',state.sort,'price'],['quality',state.quality,'all'],['q',state.search,'']])if(value&&value!==defaultValue)url.searchParams.set(key,value);
  return url.href;
}
