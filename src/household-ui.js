...
export function setupHousehold(form){
  ...
  <details class="gifts-panel"><summary id="gifts-title">증여받는 돈이 있어요</summary><p class="field-help">현금 칸에는 아래 외부 증여금을 제외한 돈을 입력하세요. 배우자 간 증여는 이체 전 현금을 입력하며 부부 총자금에 두 번 더하지 않습니다.</p>${giftFields('giftSelf','내가 받는 증여')}<div id="spouse-gift" hidden>${giftFields('giftSpouse','배우자가 받는 증여')}</div><p class="field-help">각자 이번 증여자 1개 그룹을 계산합니다. 세금은 받는 사람이 부담하고 구매 자금에서 미리 빼둡니다. 여러 그룹의 동시 증여·증여자의 세금 대납·기한 후 가산세는 별도 계산 대상입니다. <a href="https://www.nts.go.kr/nts/cm/cntnts/cntntsView.do?cntntsId=7960&mi=6533" target="_blank" rel="noopener noreferrer">국세청 계산 기준 ↗</a></p></details><div id="funding-summary" class="funding-summary" aria-live="polite"></div>`);
