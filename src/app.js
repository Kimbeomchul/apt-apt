...
function detail(id){
  ...
  $('#modal-body .detail-actions')?.insertAdjacentHTML('beforeend',`<button class="outline-button" data-share-apartment="${escape(a.id)}">공유하기</button>`);
...
async function shareApartment(id){
  const url=new URL(location.href);url.searchParams.set('apartment',id);const value=url.href;
  try{await navigator.clipboard.writeText(value);toast('단지 판단 링크를 복사했어요.');}catch{openModal('단지 공유 링크',`<label class="field">링크<input class="share-url" type="text" readonly value="${escape(value)}"></label><p>링크를 길게 눌러 복사해 주세요.</p>`,'SHARE HOME');}
}
document.addEventListener('click',event=>{
  ...
  if(b.dataset.shareApartment){shareApartment(b.dataset.shareApartment);return;}
