...
function readProfile(){const f=new FormData(form),p={};for(const [k,v] of f)p[k]=['homeStatus','purchaseHistory'].includes(k)?v:v==='on'?true:v.trim()===''?NaN:Number(v);p.depositReady=f.has('depositReady');p.creditRestriction=f.has('creditRestriction');return p;}
...
function renderFilterSummary(){
  ...
  if(state.affordable)parts.push('예산 안');
...
function clearFilters(){state.region='all';state.area='84';state.search='';state.sort='price';state.affordable=false;state.favoritesOnly=false;$('#search').value='';$('#sort').value='price';$('#affordable').checked=false;$('#favorites-only').checked=false;setPressed('data-region','all');setPressed('data-area','84');renderList();}
...
form.addEventListener('submit',e=>{e.preventDefault();const p=readProfile(),errors=validateProfile(p);$('#form-error').textContent=errors.join(' ');if(errors.length){$('#form-error').focus({preventScroll:false});return;}state.profile=p;renderBudget();renderList();toast('입력한 조건으로 다시 계산했어요.');});
form.addEventListener('reset',()=>{setTimeout(()=>{state.profile=readProfile();$('#form-error').textContent='';$('#history-error').textContent='';form.elements.purchaseHistory.removeAttribute('aria-invalid');renderBudget();renderList();},0);});
for(const name of ['homeStatus','purchaseHistory'])form.elements[name].addEventListener('change',()=>{
  const p=readProfile(),issues=profileIssues(p);
  $('#history-error').textContent=issues.join(' ');
  form.elements.purchaseHistory.setAttribute('aria-invalid',String(issues.length>0));
  const errors=validateProfile(p);$('#form-error').textContent=errors.join(' ');
  // Invalidate stale amounts immediately when the eligibility choice changes.
  state.profile={...state.profile,homeStatus:p.homeStatus,purchaseHistory:p.purchaseHistory};renderBudget();renderList();
});
